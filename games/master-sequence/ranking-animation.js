 ...
 * 1. Entender que hay un leaderboard
 * 2. Ver claramente su posición
 * 3. Motivarse a ingresar su nombre para guardar el score
 */
...
(function() {
    ...
    async function showRankingAnimation(playerScore, targetContainer) {
        ...
        // Insertar DESPUÉS del final-stats (antes del name input)
        const finalStats = targetContainer.querySelector('.final-stats');
        if (finalStats && finalStats.nextElementSibling) {
            targetContainer.insertBefore(container, finalStats.nextElementSibling);
        } else if (finalStats) {
            finalStats.after(container);
        } else {
            const nameInputDiv = targetContainer.querySelector('[for="gameOverPlayerNameInput"]')?.parentElement;
            if (nameInputDiv) {
    ...
    console.log('✅ [ranking-animation.js] Ranking animation component loaded');
